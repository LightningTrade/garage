...
class GaussianMLPEmbedding(StochasticEmbedding):
    ...
    def latent_sym(self, in_var, name='latent_sym'):
        """Build a symbolic graph of the latent parameters.
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            mean_var, log_std_var, _, _ = self.model.build(in_var, name=name)
            latent_dist = tf.contrib.distributions.MultivariateNormalDiag(
                mean_var, log_std_var)
            latent_var = latent_dist.sample(seed=get_seed())
        return latent_var

    def entropy_sym(self, in_var, name='entropy_sym'):
        """Build a symbolic graph of the entropy.
        ...
        Returns:
            dict[tf.Tensor]: Outputs of the symbolic graph of entropy
                parameters.
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            ...
            latent_dist = tf.contrib.distributions.MultivariateNormalDiag(
                mean_var, log_std_var)
        return latent_dist.entropy()

    def log_likelihood_sym(self, input_var, latent_var, name='log_likelihood_sym'):
        with tf.compat.v1.variable_scope(self._variable_scope):
            mean_var, log_std_var, _, _ = self.model.build(input_var, name=name)
            latent_dist = tf.contrib.distributions.MultivariateNormalDiag(
                mean_var, log_std_var)
        return latent_dist.log_prob(latent_var)
